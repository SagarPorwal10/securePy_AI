A crafted, terminal-native console for **SecurePy AI** — no gradient hero, no glass cards, no emoji boxes. Built like a real security product: live scan console, findings workbench, pipeline, research narrative, and the 24-phase roadmap.
...
/* ================================================================
   ...
   A terminal-native, editorial product surface.
   ================================================================ */
...
function Scramble({ text, className, delay = 0 }) {
  ...
  useEffect(() => {
    ...
    if (!inView) return;
    const chars = "█▓▒░<>/\\|=+*#01";
    let frame = 0; let raf; let started = false;
    const total = 26;
    const tick = () => {
      frame++;
...
function ASTTree() {
  ...
  return (
    <svg viewBox="0 0 400 320" className="sy-ast" aria-hidden="true">
      {segs.map((s, i) => (
...
export default function App() {
  ...
  return (
    <div className="sy-root">
      ...
      {/* status bar */}
      <div className="sy-status">
        <span className="sy-status-item"><span className="sy-live" />ollama · connected</span>
      ...
      <header className="sy-nav">
        ...
        <a className="sy-cta" href="#console">run_scan --fix</a>
      </header>

      {/* opening: editorial + live console */}
      <section className="sy-open" id="top">
        <div className="sy-open-left">
          <div className="sy-kicker">SAST × LOCAL-LLM REMEDIATION × CI GATE</div>
          <h1 className="sy-h1">
          ...
          <div className="sy-open-meta">
            <span className="sy-chip">9 phases shipped</span>
            <span className="sy-chip">5 CWE engines</span>
            <span className="sy-chip">3 report formats</span>
            <span className="sy-chip sy-chip-g">privacy-first</span>
          </div>
        </div>
        <div className="sy-open-right">
          <Terminal />
          <div className="sy-open-strip">
      ...
      {/* pipeline */}
      <section className="sy-sec" id="pipeline">
        <SectionHead index="01" label="DETECTION → REMEDIATION PIPELINE" title="Eight stages. One verified patch." />
        <div className="sy-pipe">
          <div className="sy-pipe-list">
            {PIPELINE.map((p, i) => (
              <button key={p.n} className={"sy-pipe-row" + (step === i ? " on" : "")} onClick={() => setStep(i)}>
          ...
          <div className="sy-pipe-detail">
            <AnimatePresence mode="wait">
              <motion.div key={step} initial={prm ? {} : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
      ...
      {/* findings console */}
      <section className="sy-sec" id="console">
        ...
        <div className="sy-filters">
          {["ALL", "CRITICAL", "HIGH", "MEDIUM"].map((s) => (
            <button key={s} className={"sy-fbtn" + (filter === s ? " on" : "")} onClick={() => setFilter(s)}>{s}</button>
          ))}
        </div>

        <div className="sy-console">
          <div className="sy-clist">
            {shown.map((x) => {
              const real = FINDINGS.indexOf(x);
              return (
                <button key={x.id} className={"sy-crow" + (sel === real ? " on" : "")} onClick={() => setSel(real)}>
          ...
          <div className="sy-cdetail">
            <div className="sy-cd-head">
              ...
              <div className="sy-cd-conf">
                <svg viewBox="0 0 36 36" className="sy-ring">
            ...
            <div className="sy-cd-modes">
              {["diff", "before", "after"].map((m) => (
                <button key={m} className={"sy-mbtn" + (mode === m ? " on" : "")} onClick={() => setMode(m)}>{m}</button>
              ))}
              <span className={"sy-cd-status sy-st-" + f.status.toLowerCase()}>{f.status}</span>
            </div>

            <div className="sy-code">
            ...
            <div className="sy-checks">
              {f.checks.map(([label, st]) => (
                <div key={label} className="sy-check">
                  <span className={"sy-check-ic sy-check-" + st}>{st === "pass" ? "✓" : st === "warn" ? "!" : "✕"}</span>
                  <span>{label}</span>
                  <span className="sy-check-st">{st.toUpperCase()}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="sy-sec sy-research" id="research">
      ...
      {/* roadmap */}
      <section className="sy-sec" id="roadmap">
        <SectionHead index="04" label="EVOLUTION · 24 PHASES" title="From thesis tool to autonomous AppSec engineer." />
        <div className="sy-road">
          {ROADMAP.map((r) => (
            <div key={r.range} className={"sy-road-row sy-road-" + r.status}>
...
/* ================= CSS ================= */
const CSS = `
@import url('https://cdn.jsdelivr.net/npm/@fontsource/space-grotesk@5.0.16/500.css');
...
@import url('https://cdn.jsdelivr.net/npm/@fontsource/ibm-plex-mono@5.0.13/500.css');
@import url('https://cdn.jsdelivr.net/npm/@fontsource/ibm-plex-sans@5.0.13/400.css');
@import url('https://cdn.jsdelivr.net/npm/@fontsource/ibm-plex-sans@5.0.13/600.css');
...
.sy-ring{width:56px;height:56px;transform:rotate(0deg)}
...
**What makes it feel hand-built, not generated:** a terminal-first opening instead of a hero; IBM Plex Mono + Space Grotesk pairing; hairline grid structure with sharp corners; scramble-decoded section labels; line-mask headline reveal; a live streaming scan console; an interactive 8-stage pipeline; a dense findings workbench with diff/validate panes; a sticky research column with a self-drawing AST tree; a 24-phase roadmap ledger; and a scanline/noise overlay — all honoring `prefers-reduced-motion`.